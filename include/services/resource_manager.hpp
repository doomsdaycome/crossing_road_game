 ...
 public:
  ...
  const sf::Texture& getTexture(const std::string& filepath) {
    ...
    auto [insertedIt, success] =
        textureCache_.emplace(filepath, std::move(texture));
    (void)success;
    return insertedIt->second;
  }

  const sf::Texture& getTextureWithMask(const std::string& filepath,
                                        sf::Color maskColor) {
    // Tạo khóa cache riêng biệt để tránh nhầm lẫn với bản không mask
    std::string cacheKey =
        filepath + "_masked_" + std::to_string(maskColor.toInteger());
    ...
    auto [insertedIt, success] =
        textureCache_.emplace(cacheKey, std::move(texture));
    (void)success;
    return insertedIt->second;
  }
  ...
  const sf::SoundBuffer& getSoundBuffer(const std::string& filepath) {
    ...
    auto [insertedIt, success] =
        soundBufferCache_.emplace(filepath, std::move(buffer));
    (void)success;
    return insertedIt->second;
  }
  ...
  void playMusic(const std::string& filepath, bool loop = true) {
    ...
    if (currentMusicFilePath_ == filepath && currentMusic_ &&
        currentMusic_->getStatus() == sf::SoundSource::Status::Playing) {
      return;
    }
