...
class Lane {
 public:
  ...
  // spawnOrderIndex: thu tu sinh lane (0, 1, 2...) - dung de RAI THOI GIAN nha
  // quai dau tien, lane sinh truoc (index nho) se nha quai som hon lane sinh
  // sau (index lon).
  Lane(LaneType type, float yPosition, int direction, float speedMultiplier,
       bool isEndlessMode, int spawnOrderIndex = 0);

  // Constructor 2: LOAD GAME (phuc hoi tu du lieu da luu)
  explicit Lane(const LaneSaveData& saveData);

  // Lane khong the copy (so huu unique_ptr<Sprite> + vector<Monster> khong the
  // copy)
  ...
  // Can thiet cho CollisionSystem: cho phep "doc" danh sach quai ma khong sua
  // duoc
 ...
 private:
  ...
  // Con tro toi texture duoc cache boi ResourceManager - KHONG so huu, khong
  // can giai phong
