...
class LaneManager {
 public:
  ...
  void update(float dt, bool isEndlessMode, float viewTopEdge,
 ...
 private:
  // MOI: sinh TUNG lane mot (kieu Flappy Bird sinh tung ong theo khoang cach co
  // dinh) thay vi sinh 1 batch nhieu lane cung luc nhu ban truoc. forceRest =
  // true -> ep lane nay la LaneType::REST (dung cho vai lane dau tien).
  void spawnOneEndlessLane(bool forceRest);

  // MOI: quyet dinh loai lane tiep theo (ROAD/GRASS/REST) - ket hop random co
  // trong so va gioi han so lane nguy hiem lien tiep, giong cach Crossy Road
  // dam bao man choi luon "choi duoc" (khong bao gio bi bit duong bat kha thi).
  ...
  // Bo dem thu tu sinh lane - dung de RAI THOI GIAN nha quai giua cac lane (xem
  // Lane::Lane)
  int spawnOrderCounter_ = 0;

  // MOI: dem so lane nguy hiem (ROAD/GRASS) da sinh LIEN TIEP ke tu lane nghi
  // chan gan nhat
