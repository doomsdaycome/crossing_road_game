...
MenuState::MenuState() {
  ...
  const auto& bgTex =
      ResourceManager::instance().getTexture(Config::MENU_BG_TEXTURE);
  ...
  for (size_t i = 0; i < configs.size(); ++i) {
    ...
    const sf::Texture& tex =
        ResourceManager::instance().getTexture(configs[i].textureId);
...
void MenuState::processEvents(Game* game,
...
void MenuState::update(Game* game, float dt) {
  ...
  for (auto& btn : m_buttons_) {
    if (isMouseHovering(*(btn.sprite), game->getWindow())) {
      ...
      std::uint8_t alpha =
          180 + static_cast<std::uint8_t>(std::abs(std::sin(time * 10.f)) * 75);
