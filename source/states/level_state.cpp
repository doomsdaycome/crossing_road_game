...
LevelState::LevelState() {
  const auto& bgTex =
      ResourceManager::instance().getTexture(Config::OPTION_BG_TEXTURE);
...
void LevelState::buildLevelButtons() {
  ...
  const sf::Texture& squareTex =
      ResourceManager::instance().getTexture(Config::BTN_BLANK_SQUARE_TEXTURE);
  ...
  for (int i = startLevel; i <= endLevel; ++i) {
    ...
    std::string filepath = Config::LEVEL_PATH_PREFIX + std::to_string(i) +
    ...
    coinText.setString(std::to_string(levelData.collectedCoins) + "/" +
                       std::to_string(levelData.totalCoins));
    ...
    coinText.setOrigin({bounds.position.x + bounds.size.x / 2.f,
  ...
  if (m_currentPage_ > 0) {
    ...
    prevBtn.text.setOrigin({bounds.position.x + bounds.size.x / 2.f,
                            bounds.position.y + bounds.size.y / 2.f});
  ...
  const sf::Texture& rectTex =
      ResourceManager::instance().getTexture(Config::BTN_BLANK_TEXTURE);
  ...
  backBtn.text.setOrigin({backBounds.position.x + backBounds.size.x / 2.f,
  ...
  if (endLevel < m_totalLevels_) {
    ...
    nextBtn.text.setOrigin({bounds.position.x + bounds.size.x / 2.f,
                            bounds.position.y + bounds.size.y / 2.f});
...
void LevelState::processEvents(Game* game,
...
void LevelState::update(Game* game, float dt) {
  if (m_transition_.isBusy()) {
    ...
    if (m_transition_.isFinishedClosing()) {
      game->changeState(new PlayingState(GameMode::CLASSIC, m_selectedLevel_),
                        true);
    }
