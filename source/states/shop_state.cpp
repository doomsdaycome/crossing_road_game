...
void ShopState::initUI() {
    ...
    backBtn->sprite->setPosition({Config::WINDOW_WIDTH / 2.f, Config::WINDOW_HEIGHT - 80.f});
...
void ShopState::initDummyItems() {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        item.isUnlocked = m_scoreManager_.isSkinUnlocked(i); // Read from ScoreManager
        item.quantity = 1;
        ...
        // --- LOAD SKIN ANIMATION ---
        std::string texPath = "asset/images/entities/player/pl" + std::to_string(i) + ".png";
        const sf::Texture& tex = ResourceManager::instance().getTextureWithMask(texPath, sf::Color::Magenta);
        ...
        // Chỉnh thời gian mỗi frame từ 0.15f lên 0.35f để nhân vật đi tản bộ chậm rãi hơn
...
void ShopState::processEvents(Game* game, const std::optional<sf::Event>& event) {
    if (const auto* mousePress = event->getIf<sf::Event::MouseButtonPressed>()) {
        if (mousePress->button == sf::Mouse::Button::Left) {
            ...
            std::vector<ShopItem>& currentItems = (m_currentPage_ == ShopPage::SKINS) ? m_skins_ : m_buffs_;
...
void ShopState::renderItemCard(sf::RenderWindow& window, const ShopItem& item, const sf::Font& font) {
    ...
    cardShape.setOutlineColor(item.isUnlocked && item.category == ItemCategory::SKIN ? sf::Color::Green : sf::Color::White);
    ...
    nameText.setPosition({item.cardRect.position.x + 10.f, item.cardRect.position.y + 10.f});
    window.draw(nameText);

    // Draw price or status
    sf::Text statusText(font);
    statusText.setCharacterSize(18);
    statusText.setPosition({item.cardRect.position.x + 10.f, item.cardRect.position.y + 110.f});
...
void ShopState::render(sf::RenderWindow& window) {
    ...
    const auto& currentItems = (m_currentPage_ == ShopPage::SKINS) ? m_skins_ : m_buffs_;
