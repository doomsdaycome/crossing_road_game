...
LoadState::LoadState() {
    ...
    for (int i = 1; i <= 3; ++i) {
        ...
        if (slot.snapshotData.has_value()) {
            ...
            slot.details.setString("Score: " + std::to_string(slot.snapshotData->score));
        } else {
            slot.isEmpty = true;
            slot.details.setFillColor(sf::Color(150, 150, 150)); // Màu xám cho ô trống
...
void LoadState::processEvents(Game* game, const std::optional<sf::Event>& event) {
            ...
            for (const auto& slot : m_slots_) {
                if (isClicked(slot.bg, worldPos)) {
                    ...
                    if (!slot.isEmpty && slot.snapshotData.has_value()) {
                        ...
                        // Xóa sạch các state Menu/Load hiện tại và nạp PlayingState mới kèm theo cục dữ liệu!
                        // LƯU Ý: Phải code thêm một constructor cho PlayingState để nhận GameSnapshot
                        game->changeState(new PlayingState(slot.snapshotData.value()), true); 
                    }
                    return; 
                }
            }
