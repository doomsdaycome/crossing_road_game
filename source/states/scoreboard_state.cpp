...
ScoreboardState::ScoreboardState() {
  ...
  // Tùy chọn: Nếu ảnh của ông nhỏ/to hơn màn hình thì uncomment 3 dòng dưới để
  // fit vừa cửa sổ float scaleX = Config::WINDOW_WIDTH /
  // m_bgSprite_->getGlobalBounds().size.x; float scaleY = Config::WINDOW_HEIGHT
  // / m_bgSprite_->getGlobalBounds().size.y;
  ...
  m_bestScoreText_->setPosition(
      sf::Vector2f{Config::WINDOW_WIDTH / 2.f, 300.f});
  ...
  m_totalGoldText_->setPosition(
      sf::Vector2f{Config::WINDOW_WIDTH / 2.f, 400.f});
  ...
  if (bestScore >= 500) {
    rankStr = "RANK: S (LEGEND)";
    rankColor = sf::Color(255, 50, 50);
  } else if (bestScore >= 300) {
    rankStr = "RANK: A (PRO)";
    rankColor = sf::Color(255, 150, 0);
  } else if (bestScore >= 100) {
    rankStr = "RANK: B (VETERAN)";
    rankColor = sf::Color(50, 255, 50);
  }
  ...
  m_backBtnSprite_->setPosition(
      sf::Vector2f{Config::WINDOW_WIDTH / 2.f, 600.f});
...
void ScoreboardState::processEvents(Game* game,
