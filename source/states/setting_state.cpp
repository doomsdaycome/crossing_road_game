...
SettingState::SettingState(bool isFromGameplay, GameSnapshot snapshot)
  ...
  int currentMusic =
      static_cast<int>(ResourceManager::instance().getMusicVolume());
  ...
  musicMuteBtn.sprite->setPosition(
      {centerX + 350.f, startY + 20.f});  // Nằm bên phải nút >
  ...
  if (m_isFromGameplay_) {
    ...
    saveBtn.action =
        ButtonAction::SaveGame;  // KHAI BÁO ACTION NÀY VÀO ENUM NHÉ
...
void SettingState::processEvents(Game* game,
        ...
        if (isClicked(*(btn.sprite), worldPos)) {
          ...
          switch (btn.action) {
            case ButtonAction::MusicDown:
              ...
              m_musicValueText_->setString(
                  std::to_string((int)rm.getMusicVolume()) + "%");
              centerOrigin(*m_musicValueText_);
            ...
            case ButtonAction::MusicMute:
              ...
              if (currentVol > 0.f) {
                ...
                rm.setMusicVolume(m_lastMusicVol_ > 0.f ? m_lastMusicVol_
                                                        : 1.f);
            ...
            case ButtonAction::SfxDown:
              ...
              m_sfxValueText_->setString(
                  std::to_string((int)rm.getSfxVolume()) + "%");
              centerOrigin(*m_sfxValueText_);
