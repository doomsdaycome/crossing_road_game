...
GameOverState::GameOverState(int score, GameMode mode, int level,
                             bool isVictory, int earnedGold, int bestScore,
                             int totalCoins)
    : m_finalScore_(score),
      m_mode_(mode),
      m_level_(level),
      m_isVictory_(isVictory),
      m_earnedGold_(earnedGold),
      m_displayScore_(0.f),
      m_hasCollided_(false),
      m_shakeTimer_(0.f) {
  ...
  m_titleText_->setFillColor(m_isVictory_ ? sf::Color(255, 215, 0)
                                          : sf::Color(220, 20, 20));
  ...
  retryBtn.text.setString((m_isVictory_ && m_mode_ == GameMode::CLASSIC)
                              ? "NEXT LEVEL"
                              : "PLAY AGAIN");
...
void GameOverState::update(Game* game, float dt) {
  ...
  if (m_hasCollided_) {
    // Đếm Điểm
    float scoreSpeed = std::max(
        100.f, m_finalScore_ * 1.5f);  // Tốc độ đếm linh hoạt theo tổng điểm
    if (m_displayScore_ < m_finalScore_) {
      m_displayScore_ += scoreSpeed * dt;
      if (m_displayScore_ > m_finalScore_)
        m_displayScore_ = static_cast<float>(m_finalScore_);

      m_scoreText_->setString(
          "SCORE: " + std::to_string(static_cast<int>(m_displayScore_)));
      centerOrigin(
          *m_scoreText_);  // Gọi lại centerOrigin để căn giữa lại khi số dài ra
    }
    ...
    if (m_displayGold_ < targetGold) {
      ...
      m_goldText_->setString("GOLD: " +
                             std::to_string(static_cast<int>(m_displayGold_)));
      centerOrigin(*m_goldText_);
    }
  }
  ...
  // ==========================================
  m_titleText_->setPosition(
      {m_leftX_, 150.f});  // Đẩy tiêu đề cao lên tí lấy chỗ trống
      ...
      for (const auto& btn : m_buttons_) {
        if (isClicked(*(btn.sprite), worldPos)) {
          if (btn.action == ButtonAction::Restart) {
            ...
            int targetLevel = (m_isVictory_ && m_mode_ == GameMode::CLASSIC)
                                  ? (m_level_ + 1)
                                  : m_level_;
