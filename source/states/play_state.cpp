...
PlayingState::PlayingState(GameMode gameMode, int level) {
  ...
  const sf::Texture& overlayTex =
      ResourceManager::instance().getTexture(Config::TOP_OVERLAY_TEXTURE);
  m_topOverlaySprite_ = std::make_unique<sf::Sprite>(overlayTex);

  const sf::Font& mainFont =
      ResourceManager::instance().getFont(Config::MAIN_FONT);
  ...
  if (m_mode_ == GameMode::CLASSIC) {
    std::string filepath = Config::LEVEL_PATH_PREFIX +
                           std::to_string(m_currentLevel_) +
                           Config::LEVEL_PATH_SUFFIX;
    ...
    const sf::Texture& finishTex =
        ResourceManager::instance().getTexture(Config::FINISH_LINE_TEXTURE);
...
PlayingState::PlayingState(const GameSnapshot& snapshot) {
  ...
  float cameraBottom =
      m_camera_.getView().getCenter().y + (Config::WINDOW_HEIGHT / 2.f);
...
void PlayingState::processEvents(Game* game,
  ...
  if (const auto* keyPress = event->getIf<sf::Event::KeyPressed>()) {
    ...
    if (keyPress->code == sf::Keyboard::Key::A ||
        ...
        keyPress->code == sf::Keyboard::Key::D ||
    ...
    if (keyPress->code == sf::Keyboard::Key::Num1) {
      ...
      m_player_.getBuffManager().consumeInvisibilityItem(
          5.f);  // Invisibility 5s
...
void PlayingState::update(Game* game, float dt) {
  ...
  if (m_isVictory_) {
    ...
    if (m_victoryTimer_ > 1.5f) {
      ...
      game->changeState(
          new GameOverState(m_score_.getScore(), m_mode_, m_currentLevel_, true,
                            m_coinsCollectedInRun_, m_score_.getBestScore(),
                            m_score_.getTotalCoins()));
    }
    return;
  }
  ...
  if (m_isCameraDeath_ || m_isMonsterDeath_) {
    ...
    if (m_shakeTimer_ <= 0.f) {
      ...
      game->pushState(
          new GameOverState(m_score_.getScore(), m_mode_, m_currentLevel_,
                            false, m_coinsCollectedInRun_,
                            m_score_.getBestScore(), m_score_.getTotalCoins()));
    }
    return;
  }
  ...
  if (m_flashAlpha_ > 0.f) {
    ...
    m_whiteFlashRect_.setFillColor(
        sf::Color(255, 255, 255, static_cast<std::uint8_t>(m_flashAlpha_)));

    m_whiteFlashRect_.setSize({static_cast<float>(Config::VIEW_WIDTH),
                               static_cast<float>(Config::VIEW_HEIGHT)});
  ...
  m_coinsCollectedInRun_ +=
      CollisionSystem::checkAndCollectCoins(m_player_, m_laneManager_);
  ...
  if (m_mode_ == GameMode::ENDLESS) {
    ...
    if (m_hudText_) {
      m_hudText_->setString(
          "SCORE: " + std::to_string(m_score_.getScore()) +
  ...
  // PHÂN NHÁNH B: CLASSIC MODE
    ...
    if (m_hudText_) {
      m_hudText_->setString(
          "LEVEL: " + std::to_string(m_currentLevel_) +
          "    COINS: " + std::to_string(m_coinsCollectedInRun_));
    }

    // Check chạm đích
    float playerWorldY =
        static_cast<float>(m_player_.getGridY() * Config::TILE_SIZE);
  ...
  int counts[4] = {m_player_.getBuffManager().getMagnetCount(),
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    if (counts[i] <= 0) {
      ...
      m_buffIcons_[i].setFillColor(
          sf::Color::White);  // Không tô màu đè lên ảnh
  ...
  if (isGameStarted_) {
    ...
    if (m_mode_ == GameMode::CLASSIC &&
        m_camera_.getTopEdge() <= m_finishLineY_ - 50.f) {
    ...
    if (shouldScrollCamera) {
      ...
      // Ông có thể thay Config::WINDOW_HEIGHT bằng Config::VIEW_HEIGHT tùy
      // thuộc ông đang xài biến nào.
  ...
  m_laneManager_.update(dt, isEndless, m_camera_.getTopEdge(),
                        m_camera_.getBottomEdge(), isRedLight);
  ...
  if (CollisionSystem::checkPlayerVsMonsters(m_player_, m_laneManager_)) {
    ...
    float pX = static_cast<float>(m_player_.getGridX() * Config::TILE_SIZE +
                                  Config::TILE_SIZE / 2);
    float pY = static_cast<float>(m_player_.getGridY() * Config::TILE_SIZE +
                                  Config::TILE_SIZE / 2);
...
void PlayingState::initHUD() {
  m_player_.getBuffManager().setCounts(
      m_score_.getMagnetCount(), m_score_.getShieldCount(),
      m_score_.getInvisCount(), m_score_.getNovaCount());
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    const sf::Texture& tex =
        ResourceManager::instance().getTexture(texPaths[i]);
    m_buffIcons_[i].setTexture(&tex, true);

    m_buffIcons_[i].setFillColor(
        sf::Color::White);  // Trắng bóc để hiện đúng màu ảnh gốc
