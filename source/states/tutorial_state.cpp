...
TutorialState::TutorialState() : 
    ...
    m_bgMenu_ = std::make_unique<sf::Sprite>(rm.getTexture(Config::MENU_BG_TEXTURE));
    
    m_backButton_.action = ButtonAction::Back;
    m_backButton_.sprite = std::make_unique<sf::Sprite>(rm.getTexture(Config::BTN_BLANK_SQUARE_TEXTURE));
    ...
    m_topButton_.sprite = std::make_unique<sf::Sprite>(rm.getTexture(Config::BTN_BLANK_SQUARE_TEXTURE));
    centerOrigin(*(m_topButton_.sprite));
    m_topButton_.sprite->setPosition({Config::WINDOW_WIDTH - 100.f, Config::WINDOW_HEIGHT - 80.f});
    ...
    m_topButton_.text.setPosition({Config::WINDOW_WIDTH - 100.f, Config::WINDOW_HEIGHT - 80.f});
    ...
    float startX = (static_cast<float>(Config::WINDOW_WIDTH) - static_cast<float>(guideTex.getSize().x)) / 2.f;
    float startY = -323.f; 
    ...
    m_scrollView_.setCenter({Config::WINDOW_WIDTH / 2.f, m_clipY_ + m_clipH_ / 2.f});

    float totalScrollHeight = startY + static_cast<float>(guideTex.getSize().y);
    m_maxScroll_ = totalScrollHeight - static_cast<float>(Config::WINDOW_HEIGHT) + 100.f; 
    if (m_maxScroll_ < 0.f) m_maxScroll_ = 0.f;
}

void TutorialState::processEvents(Game* game, const std::optional<sf::Event>& event) {
    if (const auto* mouseEvent = event->getIf<sf::Event::MouseButtonPressed>()) {
        if (mouseEvent->button == sf::Mouse::Button::Left) {
            sf::Vector2f mousePos = game->getWindow().mapPixelToCoords(mouseEvent->position, game->getWindow().getDefaultView());
            
            if (m_backButton_.sprite->getGlobalBounds().contains(mousePos)) {
                game->popState();
            } else if (m_scrollOffset_ > 200.f && m_topButton_.sprite->getGlobalBounds().contains(mousePos)) {
                m_scrollOffset_ = 0.f;
                m_scrollView_.setCenter({Config::WINDOW_WIDTH / 2.f, m_clipY_ + m_clipH_ / 2.f});
            }
        }
        ...
        if (scrollEvent->wheel == sf::Mouse::Wheel::Vertical) {
            ...
            m_scrollView_.setCenter({Config::WINDOW_WIDTH / 2.f, m_clipY_ + m_clipH_ / 2.f + m_scrollOffset_});
        }
    }
}
