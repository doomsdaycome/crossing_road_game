...
void LaneManager::initBackground(float startY) {
  ...
  const sf::Texture& texture =
      ResourceManager::instance().getTexture(Config::BG_TEXTURE);
...
void LaneManager::initEndless() {
  ...
  // MOI: sinh cac lane con lai bang pickNextLaneType() (thong qua
  // spawnOneEndlessLane(false))
  // - tu day tro di, lane nghi chan se tiep tuc xuat hien XEN KE xuyen suot
  // toan bo qua trinh choi, khong con dung lai o vai lane dau game nua.
...
void LaneManager::buildFromSaveData(
    const std::vector<LaneSaveData>& savedLanes) {
...
LaneType LaneManager::pickNextLaneType() {
  // Luat bat buoc: da qua nhieu lane nguy hiem lien tiep -> EP phai la lane
  // nghi chan. Day la phan quan trong nhat: dam bao man choi khong bao gio bi
  // "dong" boi 1 chuoi lane quai vo tan, giong nhu Crossy Road that luon co
  // diem dung an toan dinh ky.
...
void LaneManager::spawnOneEndlessLane(bool forceRest) {
  ...
  // MOI: truyen thu tu sinh lane vao Lane de no tu rai thoi gian nha quai dau
  // tien
  lanes_.push_back(std::make_unique<Lane>(type, highestLaneY_, randomDir,
                                          globalSpeedMultiplier_, true,
                                          spawnOrderCounter_));
  ++spawnOrderCounter_;
}

void LaneManager::update(float dt, bool isEndlessMode, float viewTopEdge,
                         float viewBottomEdge, bool isRedLight) {
  if (bgSprite2_) {
  ...
  if (isEndlessMode) {
    // MOI - KIEU FLAPPY BIRD: thay vi sinh 1 batch 5 lane cung luc khi vuot
    // nguong, dung vong lap "while" de sinh TUNG lane mot, moi lan cach nhau
    // dung LANE_HEIGHT. Giong het cach Flappy Bird luon giu 1 khoang cach co
    // dinh phia truoc man hinh truoc khi sinh ong tiep theo, bat ke frame
    // rate/dt the nao.
    ...
    lanes_.erase(std::remove_if(
                     lanes_.begin(), lanes_.end(),
                     [viewBottomEdge](const std::unique_ptr<Lane>& lane) {
                       return lane->getYPosition() >
                              viewBottomEdge + Config::ENDLESS_LANE_CULL_MARGIN;
                     }),
                 lanes_.end());
  }
}
