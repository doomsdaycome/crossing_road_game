...
bool IntroCutscene::isFinishedOpening() const {
  return m_status_ == Status::IDLE;
}
bool IntroCutscene::isFinishedClosing() const {
  return m_status_ == Status::CLOSED;
}
bool IntroCutscene::isBusy() const {
  return m_status_ != Status::IDLE && m_status_ != Status::CLOSED;
}
...
void IntroCutscene::render(sf::RenderWindow& window) {
  if (m_status_ == Status::IDLE)
    return;  // Đang nằm im (cửa mở toang) thì không vẽ gì cả
  ...
  const sf::Texture& texLeft =
      ResourceManager::instance().getTexture(Config::DOOR_LEFT_TEXTURE);
  const sf::Texture& texRight =
      ResourceManager::instance().getTexture(Config::DOOR_RIGHT_TEXTURE);
  ...
  if (m_status_ == Status::LOADING) {
    ...
    head.setPosition(
        {barX + (barWidth * m_loadingProgress_) - 4.f, barY - 1.f});
    ...
    std::uint8_t alpha =
        160 + static_cast<std::uint8_t>(std::abs(std::sin(time * 6.f)) * 95);
    loadText.setFillColor(sf::Color(255, 200, 200, alpha));

    sf::FloatRect textBounds = loadText.getLocalBounds();
    loadText.setOrigin({textBounds.position.x + textBounds.size.x / 2.f,
