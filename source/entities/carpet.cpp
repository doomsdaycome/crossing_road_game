...
Carpet::Carpet(const sf::Texture& texture, float startX, float startY, float speed, int direction) {
...
void Carpet::render(sf::RenderWindow& window) {
    window.draw(*m_sprite_);
}

float Carpet::getPositionX() const {
    return m_logicalPos_.x;
}

bool Carpet::isOffScreen() const {
    // Magic Carpet is 3 TILE_SIZE wide (192px), we need a bit more margin to delete it
