...
// ==========================================
Lane::Lane(LaneType type, float yPosition, int direction, float speedMultiplier,
           bool isEndlessMode, int spawnOrderIndex)
    : type_(type),
  ...
  if (type_ == LaneType::REST) {
    ...
    if (type_ == LaneType::ROAD)
      setupWalkAssets();
    else if (type_ == LaneType::GRASS)
      setupFlyAssets();
    else if (type_ == LaneType::CHASM)
      setupChasmAssets();
    ...
    // Tính độ trễ sinh quái đầu tiên (Stagger Delay) để tránh quái kẹt thành
    // hàng dọc
    spawnTimer_ =
        static_cast<float>(spawnOrderIndex % Config::SPAWN_STAGGER_SLOTS) *
        Config::SPAWN_STAGGER_STEP;
  }
  ...
  if (isEndlessMode && type_ != LaneType::REST) {
    if (std::rand() % 100 < 35) {  // 35% tỷ lệ xuất hiện vật phẩm
      ...
      int totalColumns =
          static_cast<int>(Config::WINDOW_WIDTH / Config::TILE_SIZE);
...
Lane::Lane(const LaneSaveData& saveData)
  ...
  if (monsterTexture_ != nullptr) {
    for (float posX : saveData.monsterPositionsX) {
      monsters_.emplace_back(*monsterTexture_, posX,
                             yPosition_ + Config::TILE_SIZE / 2.f, finalSpeed,
                             direction_, monsterFrameCount_);
    }
  }

  if (carpetTexture_ != nullptr) {
    for (float posX : saveData.carpetPositionsX) {
      carpets_.emplace_back(*carpetTexture_, posX,
                            yPosition_ + Config::TILE_SIZE / 2.f, finalSpeed,
                            direction_);
    }
  }
...
void Lane::setupWalkAssets() {
  const sf::Texture& bgTex =
      ResourceManager::instance().getTexture(Config::ZOMBIE_LANE_TEXTURE);

  int randIdx = std::rand() % Config::MONSTER_WALK_COUNT;
  std::string path =
      Config::MONSTER_WALK_DIR + "mt" + std::to_string(randIdx) + ".png";
  monsterTexture_ =
      &ResourceManager::instance().getTextureWithMask(path, sf::Color::Magenta);
...
void Lane::setupFlyAssets() {
  const sf::Texture& bgTex =
      ResourceManager::instance().getTexture(Config::BAT_LANE_TEXTURE);

  int randIdx = std::rand() % Config::MONSTER_FLY_COUNT;
  std::string path =
      Config::MONSTER_FLY_DIR + "mt" + std::to_string(randIdx) + ".png";
  monsterTexture_ =
      &ResourceManager::instance().getTextureWithMask(path, sf::Color::Magenta);
...
void Lane::setupChasmAssets() {
  const sf::Texture& bgTex =
      ResourceManager::instance().getTexture(Config::CHASM_LANE_TEXTURE);
  carpetTexture_ =
      &ResourceManager::instance().getTexture(Config::CARPET_TEXTURE);
...
void Lane::update(float deltaTime, bool isRedLight) {
  if (type_ != LaneType::REST && !isRedLight) {
    ...
    if (spawnTimer_ <= 0.f && !spawnPattern_.empty()) {
      bool canSpawn = true;
      float spawnX = (direction_ == 1) ? Config::MONSTER_SPAWN_START_LEFT
                                       : Config::MONSTER_SPAWN_START_RIGHT;

      // Khóa khoảng cách (Physical Gap Lock):
      // Bắt buộc cách đuôi con trước ít nhất 1 TILE + 20px
      if (!monsters_.empty()) {
        float distanceToLast =
            std::abs(monsters_.back().getPositionX() - spawnX);
      ...
      if (canSpawn && monsterTexture_ != nullptr) {
        float finalSpeed = baseSpeed_ * speedMultiplier_;
        monsters_.emplace_back(*monsterTexture_, spawnX,
                               yPosition_ + Config::TILE_SIZE / 2.f, finalSpeed,
                               direction_, monsterFrameCount_);
      }
      ...
      if (canSpawn && carpetTexture_ != nullptr) {
        float finalSpeed = baseSpeed_ * speedMultiplier_;
        carpets_.emplace_back(*carpetTexture_, spawnX,
                              yPosition_ + Config::TILE_SIZE / 2.f, finalSpeed,
                              direction_);
      }

      // Ép ngưỡng delay tối thiểu để quái/thảm không bị đẻ ra chồng chéo khi
      // tốc độ quá cao
      float nextDelay = spawnPattern_[static_cast<size_t>(patternIndex_)];
      float delayReduction = 1.0f + (speedMultiplier_ - 1.0f) * 0.25f;

      // Do thảm dài gấp 3 lần, nên cần thời gian tối thiểu cao hơn để không bị
      // đè
      float minDelay = (type_ == LaneType::CHASM) ? 1.5f : 0.4f;
      float actualDelay = std::max(nextDelay / delayReduction, minDelay);

      // Bắt buộc reset timer và chuyển pattern kế tiếp ngay cả khi bị block
      // (canSpawn == false) Nếu không reset, spawnTimer_ sẽ mãi mãi <= 0 và
      // quái bị đẻ chồng chất vào frame tiếp theo
      spawnTimer_ = actualDelay;
      patternIndex_ =
          (patternIndex_ + 1) % static_cast<int>(spawnPattern_.size());
    }
  }
  ...
  monsters_.erase(
      ...
      monsters_.end());
  carpets_.erase(
      std::remove_if(carpets_.begin(), carpets_.end(),
                     [](const Carpet& c) { return c.isOffScreen(); }),
      carpets_.end());
}
  ...
  if (!items_.empty()) {
    ...
    for (const auto& item : items_) {
      ...
      if (item.type == ItemType::COIN_1)
        tex = &rm.getTexture(Config::COIN_1_TEXTURE);
      else if (item.type == ItemType::COIN_3)
        tex = &rm.getTexture(Config::COIN_3_TEXTURE);
      else if (item.type == ItemType::TREASURE)
        tex = &rm.getTexture(Config::TREASURE_TEXTURE);

      if (tex != nullptr) {
        sf::Sprite spr(*tex);  // Tạo thẳng trên Stack cực nhanh, tránh cấp phát
                               // bộ nhớ bừa bãi
        centerOrigin(spr);
        spr.setPosition({item.x + Config::TILE_SIZE / 2.f,
                         yPosition_ + Config::TILE_SIZE / 2.f});
        window.draw(spr);
      }
    }
  }
...
int Lane::collectItemAt(float playerX) {
  for (auto it = items_.begin(); it != items_.end();) {
    ...
    if (std::abs(it->x - playerX) < Config::TILE_SIZE / 2.0f) {
      int earned = 0;
      if (it->type == ItemType::COIN_1)
        earned = 1;
      else if (it->type == ItemType::COIN_3)
        earned = 3;
      else if (it->type == ItemType::TREASURE)
        earned = 50;
...
void Lane::clearMonsters() { monsters_.clear(); }
